...
import           Replica.VDOM.Types         (HTML, VDOM, VDOM(VNode,VLeaf,VText,VRawText), Attrs, Attr, Attr(AText,ABool,AEvent,AMap))

renderHTML :: HTML event -> TB.Builder
renderHTML = mconcat . map renderVDOM

renderVDOM :: VDOM event -> TB.Builder
...
renderAttrs :: Attrs event -> TB.Builder
renderAttrs = foldMap (TB.singleton ' ' <>) . _renderAttrs
  where
    dq = TB.singleton '"'
    eq = TB.singleton '='

    _renderAttrs :: Attrs event -> [TB.Builder]
    _renderAttrs = foldMap (uncurry _renderAttr) . M.toList

    _renderAttr :: T.Text -> Attr event -> [TB.Builder]
